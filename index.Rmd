...
This study demonstrates the use of a machine learning technique to achieve automatic assessment of quality of performance during a weight lifting  exercise using dumbbells. The Random Forests algorithm produced highly accurate results as seen by 100% accuracy obtained on the 20 test cases.   
...
We used Random Forests algorithm to build a classifier, which is briefly described below.  We explored few other algorithms such as boosting, using 'gbm' from caret package.  Since the accuracy achieved by Random Forests was always much larger than that achieved by the other algorithms, the trials with the other algorithms were discarded in the early stages of the study.   
...
We tried 8 different models with varying values of parameters. Since the Random Forests method takes a long time to execute (over 2 hours of execution time on my Windows 7 quadcore PC ), once computed the models were saved. The 8th model, RfFit8, is used to generate this report. 
...
```{r  warning = FALSE, message = FALSE}
# finding error rate for the validation data set
